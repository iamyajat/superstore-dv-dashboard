...
fig <- plot_ly(data, x = ~x, y = ~y, type = 'bar', text = text,
        marker = list(color = 'rgb(235, 190, 209)',
                      line = list(color = 'rgb(232, 76, 134)',
                                  width = 1.5)))
fig <- fig %>% layout(title = paste("Top",input$bw_adjust,"Cities in", input$bw_adjust),
         xaxis = list(title = ""),
         yaxis = list(title = ""),
         margin = c(t=100),
         height = 700)
...
--------------------------------------------------------------------------------
```{r, echo=FALSE}
#state_wise_sales_average = tapply(data$Sales, data$State, mean)
#data$shipping_time <-  as.Date(as.character(data$Ship.Date), format="%m/%d/%Y") - as.Date(as.character(data$Order.Date), format="%m/%d/%Y")
#average_shipping_time_by_province = tapply(data$shipping_time, data$State, mean)
...
```{r, echo=FALSE}
#average_shipping_time_by_mode_of_transport = tapply(data$shipping_time, data$Ship.Mode, mean)
# LINK MAP HERE
```
...
df%>%
  ...
  geom_point()+
  theme_bw()+
  labs(title = 'Technology: Sales vs Profits', color='Discount')+
